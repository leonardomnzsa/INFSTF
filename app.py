...
def configurar_openai():
    try:
        ...
        # Chave não encontrada nos segredos
        return False
...
# Função para gerar assertivas usando a API do ChatGPT
def gerar_assertivas_api(df, materias_selecionadas=None, num_assertivas=5):
    ...
    try:
        ...
        response = openai.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Você é um especialista em criar questões de concurso sobre jurisprudência do STF."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=1000, # Aumentar tokens para comportar JSON
            temperature=0.7, # Um pouco mais de criatividade para estilo de concurso
            response_format={ "type": "json_object" } # Solicitar resposta em JSON (para modelos compatíveis)
        )
        resposta_json_str = response.choices[0].message.content.strip()
        
        # Tentar parsear o JSON
        assertivas_api = json.loads(resposta_json_str)
        ...
        if isinstance(assertivas_api, list) and all(isinstance(item, dict) and all(key in item for key in ["texto", "resposta", "explicacao"]) for item in assertivas_api):
            return assertivas_api
        else:
            st.error("A resposta da API não está no formato JSON esperado. Usando simulação.")
            return gerar_assertivas_simuladas(df, materias_selecionadas, num_assertivas)
            
    except json.JSONDecodeError:
        st.error("Erro ao decodificar a resposta JSON da API. Usando simulação.")
        return gerar_assertivas_simuladas(df, materias_selecionadas, num_assertivas)
